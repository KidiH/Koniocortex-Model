%% NOTE: This computational code was adapted from a Matlab code.

function numbergraph1(w1,w2,max_m,min_m)
...
%if nargin < 2, max_m = max(max(abs(w1))); end
...
for i=1:S
  ...
  m = sqrt((abs(w1(i,j))-min_m)/max_m);
  m = min(m,max_m)*0.95;
  
  m1 = sqrt((abs(w2(i,j))-min_m)/max_m);
  m1 = min(m1,max_m)*0.95;
  ...
  if real(m)
    ...
    if w2(i,j) < 0
      fill(xn*m1+j,yn*m1+i,[0.8 0.8 0]);
        plot(xn1*m1+j,yn1*m1+i,'k',xn2*m1+j,yn2*m1+i,'w');
    end
  end
  end
end
